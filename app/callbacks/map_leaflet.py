...
def register_map_leaflet_callbacks(app, date_ranges_df):
    ...
    def update_buttons_w_counts(counts, button_ids):
        """
        Update the labels of unit filter buttons to include a badge showing the count of selected polygons.
        
        Args:
            counts: Dictionary with counts of selected polygons by unit type
            button_ids: IDs of all unit filter buttons
            
        Returns:
            list: Updated children for each unit filter button
        """
        button_children = []
        # Process each unit filter button
        for b in button_ids:
            unit = b["unit"]
            # Get the display name for the unit type
            label = UNIT_TYPES.get(unit).get("long_name", unit)
            # Get the count of selected polygons for this unit type
            count = counts.get(unit, 0)
            
            if count > 0:
                # If there are selected polygons, add a badge with the count
                children = [
                    label,
                    dbc.Badge(
                        str(count),                   # Count as string
                        color="light",                # Light background
                        text_color="dark",            # Dark text
                        pill=True,                    # Circular badge
                        className="ms-1",             # Left margin
                        style={"fontSize": "0.8em", "verticalAlign": "middle"}  # Style
                    )
                ]
            else:
                # No selected polygons, just show the label
                children = label
