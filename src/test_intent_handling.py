...
class TestIntentHandling(unittest.TestCase):
    ...
    def _assert_intents_match(
        ...
        self.assertCountEqual(
            actual_intents, expected_intents, f"Intent mismatch for query: '{query}'"
        )
        ...
        if expected_places is not None:
            ...
            self.assertCountEqual(
                actual_places,
                [p.lower() for p in expected_places],
        ...
        if expected_theme is not None:
            ...
            self.assertTrue(
                any(expected_theme.lower() in theme for theme in theme_intents),
    ...
    def test_basic_stats_patterns(self):
        ...
        for query, expected_intents, expected_places, expected_theme in test_cases:
            with self.subTest(query=query):
                self._assert_intents_match(
                    query, expected_intents, expected_places, expected_theme
                )
    ...
    def test_multiple_places(self):
        ...
        test_cases = [
            ...
            (
                ...
                ["leeds", "sheffield", "liverpool", "nottingham"],
                "housing",
            ),
        ]
        ...
        for query, expected_intents, expected_places, expected_theme in test_cases:
            with self.subTest(query=query):
                ...
                for intent in expected_intents:
                    self.assertIn(
                        intent, actual_intents, f"Missing {intent} in query: '{query}'"
                    )
    ...
    def test_postcode_handling(self):
        ...
        for query, expected_intents in test_cases:
            with self.subTest(query=query):
                ...
                full_intents = self._extract_full_intents(query)
                add_place_intents = [
                    i for i in full_intents if i["intent"] == AssistantIntent.ADD_PLACE
                ]
                if add_place_intents:
                    # Check that at least one has a postcode argument
                    has_postcode = any(
                        "postcode" in i["arguments"] for i in add_place_intents
                    )
                    self.assertTrue(
                        has_postcode, f"No postcode argument found for query: '{query}'"
                    )
