...
def register_map_state_routes(server):
    ...
    def toggle_polygon():
        ...
        try:
            ...
            # Get current selections using helper functions (derive from places)
            from vobchat.state_schema import get_selected_units, get_selected_unit_types
            places = state.get("places", []) or []
            current_place_objects = list(places)  # Work with a copy
            current_units = get_selected_units(state)
            current_unit_types = get_selected_unit_types(state)
            current_places = [place.get("g_place") for place in places if place.get("g_place")]
            ...
            if action == "add":
                ...
                if polygon_id_int not in current_units:
                    # Add to places array (single source of truth)
                    place_obj = {
                        "name": name,
                        "candidate_rows": [],
                        "g_place": None,
                ...
                if polygon_id_int in current_units:
                    ...
                    # Polygons are now derived from places array
            ...
            # Return the updated selection state (places as single source of truth)
            response_data = {
                "success": True,
                ...
                "name": name,
                "places": current_place_objects,  # Single source of truth
                "total_selected": len(current_place_objects)
            }
    ...
    def get_map_state(thread_id: str):
        ...
        try:
            ...
            if state_data:
                state = json.loads(state_data)
                response_data = {
                    "places": state.get("places", []),  # Single source of truth
                    "total_selected": len(state.get("places", []))
                }
            else:
                response_data = {
                    "places": [],  # Single source of truth
    ...
    def clear_map_selection(thread_id: str):
        ...
        try:
            ...
            if state_data:
                ...
                state.update({
                    "places": [],  # Single source of truth (clears all polygons)
    ...
    def handle_user_action():
        ...
        try:
            ...
            logger.warning(f"LOOP DEBUG: User action received: {action} with data: {action_data}")
