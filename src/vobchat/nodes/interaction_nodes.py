...
Provides a generic clarification step when the agent cannot map a message to a
...
message which is then routed by the conversational agent as usual.
"""
...
def ask_followup_node(state: lg_State) -> dict | Command:
    ...
    When the conversational agent cannot map the user message to any of the
    ...
    • When the user clicks one of those buttons the front-end writes
      ...
      back to the normal intent router (via ``start_node`` which leads to the conversational agent).
    ...
    Returns:
        ...
        is selected, returns a ``Command`` that routes to the main router (``start_node``) with a
    ...
    if state.get("selection_idx") is not None:
        try:
            ...
            return Command(goto="start_node", update=update_dict)
