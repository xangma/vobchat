...
def serialize_messages(messages):
    ...
    for msg in messages:
        if isinstance(msg, AIMessage):
            serialized.append({"_type": "ai", "content": msg.content, "type": "ai"})
        elif isinstance(msg, HumanMessage):
            serialized.append(
                {"_type": "human", "content": msg.content, "type": "human"}
            )
...
def _append_ai(state: lg_State, text: str) -> AIMessage:
    """Create a streamable AI message and return it (do not mutate state).

    The caller should return this message as a delta via
    ``{"messages": [message]}`` to cooperate with the add_messages reducer.
    """
    message = AIMessage(
        content=text,
        response_metadata={
...
def _clean_duplicate_intents_from_queue(state: lg_State):
    ...
    if len(cleaned_queue) < original_length:
        logger.info(
            f"_clean_duplicate_intents_from_queue: Removed {original_length - len(cleaned_queue)} duplicate intents from queue"
        )
