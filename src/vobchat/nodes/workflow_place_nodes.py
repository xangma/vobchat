...
# def update_polygon_selection(state: lg_State) -> dict | Command:
#     """
#     Node that updates polygon selection state and handles routing decisions.
...
#     # # First check if all places are processed before checking for missing units
#     # has_more_places = (
...
#     logger.info(f"URGENT DEBUG: update_polygon_selection - units_needing_map_selection={units_needing_map_selection}")
#     logger.info(f"URGENT DEBUG: update_polygon_selection - current_place_index={current_place_index}, total_places={len(places)}")

#     # If there are units that still need map interaction, route to the
...
# def request_map_selection(state: lg_State) -> dict | Command:
#     """
...
#         logger.debug("URGENT DEBUG: No units need selection, routing to agent_node")
...
#         # Only include map_update_request if it's different from current state
#         if current_map_request != map_update_request:
#             logger.info(f"URGENT DEBUG: Including map_update_request for single place")
#             return Command(goto="agent_node", update={"map_update_request": map_update_request})
#         else:
#             logger.info(f"URGENT DEBUG: Skipping duplicate map_update_request for single place")
#             return Command(goto="agent_node", update={"map_update_request": None})




# def select_unit_on_map(state: lg_State) -> dict | Command:
#     """
...
#                 logger.info(f"select_unit_on_map: All places processed ({current_place_index} >= {len(places)}), proceeding to routing")

#                 # Route based on final state (moved from select_unit_on_map_router)
#                 selected_workflow_units = get_selected_units(state)
#                 if not selected_workflow_units:
#                     logger.debug("=== WORKFLOW TRACE: select_unit_on_map returning agent_node (no units) ===")
#                     logging.info("select_unit_on_map: returning agent_node (no units)")
#                     return Command(goto="agent_node")

#                 # Use normal routing logic for final decision (inline)
#                 places = state.get("places", []) or []
...
#                 if current_index is not None and current_index >= num_places:
...
#     logger.info(f"select_unit_on_map: About to exit and use routing. Current state: current_place_index={state.get('current_place_index')}, total_places={len(state.get('places', []) or [])}")

#     # Handle routing logic (moved from select_unit_on_map_router)
#     selected_workflow_units = get_selected_units(state)
#     current_place_index = state.get("current_place_index", 0)
#     places = state.get("places", []) or []
#     continue_to_next_place = state.get("continue_to_next_place", False)

#     logger.info(f"=== WORKFLOW TRACE: select_unit_on_map routing - selected_workflow_units={selected_workflow_units} ===")
#     logger.info(f"=== WORKFLOW TRACE: select_unit_on_map routing - current_place_index={current_place_index}, total_places={len(places)} ===")
#     logger.info(f"=== WORKFLOW TRACE: select_unit_on_map routing - continue_to_next_place={continue_to_next_place} ===")
...
#     if continue_to_next_place:
#         logger.info(f"=== WORKFLOW TRACE: select_unit_on_map - continue_to_next_place=True, routing to resolve_place_and_unit ===")
#         logging.info("select_unit_on_map: continue_to_next_place flag set, clearing it and continuing to resolve_place_and_unit")
...
#         logger.info(f"=== WORKFLOW TRACE: select_unit_on_map returning resolve_place_and_unit (place {current_place_index} of {num_places} still needs processing) ===")
#         logging.info(f"select_unit_on_map: returning resolve_place_and_unit (place {current_place_index} of {num_places} still needs processing)")
#         return Command(goto="resolve_place_and_unit")
...
#         if theme_intents:
#             logging.info(f"select_unit_on_map: All places processed, now handling AddTheme intents {theme_intents}")
#             return Command(goto="resolve_theme")
#         else:
#             logging.info(f"select_unit_on_map: returning agent_node (non-theme intent queue: {intent_queue})")
#             return Command(goto="agent_node")
...
# def resolve_place_and_unit(state: lg_State) -> dict | Command:
#     """
...
#         logger.error(f"🔍 resolve_place_and_unit COMMAND UPDATE (all places done)")
#         return Command(goto="agent_node")
...
#                         return Command(goto="update_polygon_selection", update={
#                             "places": places,
#                             "selection_idx": None,  # Clear UI selection
#                             "options": None,  # Clear UI options
...
#             return Command(goto="agent_node", update={"current_place_index": i + 1})
...
#                         "messages": state.get("messages", []),  # CRITICAL: Include messages to persist the confirmation message
#                         "selection_idx": None,  # Clear UI selection
#                         "options": None,  # Clear UI options
#                         "current_node": None,  # Clear current node
#                     }
#                     logger.error(f"🔍 resolve_place_and_unit COMMAND UPDATE: {list(update_dict.keys())}")
#                     return Command(goto="update_polygon_selection", update=update_dict)
#                 else:
...
#         "selection_idx": None,  # Clear UI selection
#         "options": None,  # Clear UI options
#         "messages": state.get("messages", []),  # CRITICAL: Include messages to persist any confirmation messages
#     }
